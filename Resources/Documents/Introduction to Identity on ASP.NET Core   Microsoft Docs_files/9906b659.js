...
function getSafeTextContent(elt) {
    if (!elt.querySelector('.sxs-lookup')) {
        return elt.textContent;
    }
    var clone = elt.cloneNode(true);
    var sxs = clone.querySelectorAll('.sxs-lookup');
    for (var i = 0; i < sxs.length; i++) {
        sxs[i].parentElement.removeChild(sxs[i]);
    }
...
function renderInTopicTOC() {
    ...
    for (var _i = 0, headings_1 = headings; _i < headings_1.length; _i++) {
        ...
        var text = getSafeTextContent(heading);
...
function relativeToGlobal(relativePath) {
    if (globalScriptTag) {
        ...
        return globalScriptTag.src.replace(/js\/(?:(\w+)(?:\.))?global\.min.*$/, replacement);
    }
    return relativePath;
}
...
function handleHeadings(container) {
    ...
    headings.forEach(function (heading) {
        if (heading.parentElement.matches("" + allowParents.join(','))) {
            ...
            var id = heading.id || getSafeTextContent(heading).split(' ').join('-');
...
function wrapContentTables() {
    if (!(msDocs.data.pageTemplate.toLowerCase() === 'conceptual'))
        return;
